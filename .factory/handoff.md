# Dinner Binder handoff — independent verification 3

Work order: `cookbook-print-run-verify-3`
Verified candidate: `020b414aa17239a2c2873b0264a4cdc073ca8b64`
Verified production URL: <https://cookbook-print-run.sociobot.in>
Date: 2026-08-28

**Release verdict: PASS.** Fresh independent QA confirms the live deployment matches the candidate application build and fulfils the recipe-file-to-printable-cooking-packet job. The earlier production-only PWA install defect is repaired.

## What was verified

- Clean install, 11 unit tests, TypeScript lint, and exact `npm run build` all pass.
- The production-shaped browser suite passes Azure's 404 deployment-control condition, offline reload, stale-cache protection, and a version-to-version worker update, plus 390 px/1440 px, keyboard, print, legal, console/page-error, and axe coverage.
- Independent import exercise passed schema.org JSON and Markdown flows, credit/allergen preservation, quantity scaling, backup, invalid-format/malformed/oversize recovery, safe URL handling, three-recipe free-limit recovery, and clear-all confirmation.
- The live site passes the browser audit with a registered/controlling worker and offline reload. It makes no first-load request outside its own origin and has no analytics, CDN assets, uploads, or account storage.
- Live index/JS/CSS SHA-256 values match the fresh build. The worker is source-equivalent after normalizing its intentionally build-specific cache ID. Security headers, cache policies, AVIF MIME type, and CSP are correct.
- Live mobile Lighthouse: 95 Performance, 100 Accessibility, 100 Best Practices, 100 SEO; 43 KiB transfer, LCP 1.2 s, CLS 0. Bundle budgets pass (11.2 KiB gzip JS, 4.9 KiB gzip CSS, 40 KiB largest image).

Detailed commands and exact evidence: `.factory/verification-3.md`.

## Known limits / next steps

- Browser print engines still control their own headers, footers, installed fonts, and final paper defaults; the app starts each recipe on a new sheet with 15 mm margins.
- Ingredient scaling intentionally changes leading numeric quantities only; it does not convert units or interpret prose quantities.
- Allergy notes and cooking times are supplied by the user and are not safety-verified.
- No product-code follow-up is required from this verification.

---

# Historical builder repair handoff (superseded by independent verification 3)
